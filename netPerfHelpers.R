########################################################################
# Helpers to make gof plots Revised based on Minhas et al. (2019)
...
getAmeGOF = function(ameFit, netNames=gofNames, netMethods=gofMethods){
	yDraws = ameFit$YPM[ sample(1000:length(ameFit$YPM), 100, replace=FALSE) ]
...
getNetPerfCoef = function(
	...
	perfStats$model = factor(rep(names(perfList) , each=5), levels=names(perfList))
